locals {
  absolute_deploy_local_file_path = var.deploy_local_file_path != null ? abspath(var.deploy_local_file_path) : null
}
...
resource "local_sensitive_file" "ca_pem" {
  ...
  filename        = "${local.absolute_deploy_local_file_path}/${each.value.certificate_domain}/${split(":", each.key)[1]}.ca.cer"
  depends_on      = [acme_certificate.main]
  file_permission = "0600"
}

resource "local_sensitive_file" "full_chain" {
  ...
  filename        = "${local.absolute_deploy_local_file_path}/${each.value.certificate_domain}/${split(":", each.key)[1]}.fullchain.cer"
  depends_on      = [acme_certificate.main]
  file_permission = "0600"
}

resource "local_sensitive_file" "issued_certificate_pem" {
  ...
  filename        = "${local.absolute_deploy_local_file_path}/${each.value.certificate_domain}/${split(":", each.key)[1]}.cer"
  depends_on      = [acme_certificate.main]
  file_permission = "0600"
}
